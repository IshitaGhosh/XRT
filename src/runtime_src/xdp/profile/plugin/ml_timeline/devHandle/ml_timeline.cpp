...
namespace xdp {
  ...
  class ResultBOContainer
  {
    public:
      xrt::bo  mBO;
      ResultBOContainer(void* devH, uint32_t sz)
      {
        /*
  ...
  MLTimelineDevHandleImpl::MLTimelineDevHandleImpl(VPDatabase*dB, uint32_t sz)
  ...
  MLTimelineDevHandleImpl::~MLTimelineDevHandleImpl()
  {
    xrt_core::message::send(xrt_core::message::severity_level::debug, "XRT", 
              "In destructor for ML Timeline Plugin for Client Device.");
  }

  void MLTimelineDevHandleImpl::updateDevice(void* devH)
  {
    xrt_core::message::send(xrt_core::message::severity_level::debug, "XRT", 
              "In MLTimelineDevHandleImpl::updateDevice");
    ...
    try {
       ...
       */
      mResultBOHolder = std::make_unique<ResultBOContainer>(devH, mBufSz);
    ...
    xrt_core::message::send(xrt_core::message::severity_level::debug, "XRT", 
              "Allocated buffer In MLTimelineDevHandleImpl::updateDevice");
  
  }

  void MLTimelineDevHandleImpl::finishflushDevice(void* /*hwCtxImpl*/, uint64_t implId)
  {
    std::ofstream fOut;
    ...
    xrt_core::message::send(xrt_core::message::severity_level::debug, "XRT", 
              "Using Allocated buffer In MLTimelineDevHandleImpl::finishflushDevice");
    ...
    msg1 << "Finished writing " << outFName << " in MLTimelineDevHandleImpl::finishflushDevice." << std::endl;
