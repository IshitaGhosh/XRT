...
#include "xdp/profile/plugin/gen_asm_elf/clientDev/gen_asm_elf.h"
...
namespace xdp {
  ...
  bool GenAsmElfPlugin::live = false;

  GenAsmElfPlugin::GenAsmElfPlugin()
    : XDPPlugin()
  {
    GenAsmElfPlugin::live = true;

    db->registerPlugin(this);
    //db->registerInfo(info::gen_asm_elf);
  }
  ...
  GenAsmElfPlugin::~GenAsmElfPlugin()
  {
    ...
    GenAsmElfPlugin::live = false;
  }

  bool GenAsmElfPlugin::alive()
  {
    return GenAsmElfPlugin::live;
  }

  void GenAsmElfPlugin::updateDevice(void* hwCtxImpl)
  {
#ifdef XDP_CLIENT_BUILD
    if (mHwCtxImpl) {
    ...
    DeviceDataEntry.implementation = std::make_unique<GenAsmElfClientDevImpl>(db);
    ...
    DeviceDataEntry.valid = true;
    DeviceDataEntry.implementation = std::make_unique<GenAsmElfVE2Impl>(db);
    DeviceDataEntry.implementation->setHwContext(hwContext);
    DeviceDataEntry.implementation->updateDevice(mHwCtxImpl);
#endif
  }

  void GenAsmElfPlugin::finishflushDevice(void* hwCtxImpl)
  {
    ...
    if (hwCtxImpl != mHwCtxImpl) {
      xrt_core::message::send(xrt_core::message::severity_level::debug, "XRT",
          "New Hw Context Impl passed in Gen Asm Elf Plugin.");
      return;
    }
  ...
  void GenAsmElfPlugin::writeAll(bool /*openNewFiles*/)
  {
#if defined(XDP_CLIENT_BUILD) || defined(XDP_VE2_BUILD)
