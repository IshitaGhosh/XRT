...
namespace xdp {
  ...
  bool AIEAccessPlugin::live = false;

  AIEAccessPlugin::AIEAccessPlugin()
    : XDPPlugin()
  {
    AIEAccessPlugin::live = true;

    db->registerPlugin(this);
    //db->registerInfo(info::aie_access);
  }
  ...
  AIEAccessPlugin::~AIEAccessPlugin()
  {
    ...
    AIEAccessPlugin::live = false;
  }

  bool AIEAccessPlugin::alive()
  {
    return AIEAccessPlugin::live;
  }

  void AIEAccessPlugin::updateDevice(void* hwCtxImpl)
  {
#ifdef XDP_CLIENT_BUILD
    if (mHwCtxImpl) {
    ...
    DeviceDataEntry.implementation = std::make_unique<AIEAccessClientDevImpl>(db);
    DeviceDataEntry.implementation->setHwContext(hwContext);
    DeviceDataEntry.implementation->updateDevice(mHwCtxImpl);
#endif
  }

  void AIEAccessPlugin::finishflushDevice(void* hwCtxImpl)
  {
#ifdef XDP_CLIENT_BUILD
    ...
    if (hwCtxImpl != mHwCtxImpl) {
      xrt_core::message::send(xrt_core::message::severity_level::debug, "XRT", 
          "New Hw Context Impl passed in AIE Access Plugin.");
      return;
    } 
  ...
  void AIEAccessPlugin::writeAll(bool /*openNewFiles*/)
  {
#ifdef XDP_CLIENT_BUILD
