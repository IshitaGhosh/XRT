...
#ifndef XDP_PROFILE_DEVICE_MMAPPED_ASM_H
#define XDP_PROFILE_DEVICE_MMAPPED_ASM_H

#include "xdp/profile/device/asm.h"

namespace xdp {

/** Specialization for ASM Profile IP with support for open+mmap on device driver based ASM subdevice
 */

class MMappedASM : public ASM
{
public:
  MMappedASM(Device*  handle  /* XDP Device Abstraction handle : xrt or HAL device handle */
