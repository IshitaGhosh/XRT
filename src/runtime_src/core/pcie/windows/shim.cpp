  ...
  int xclReadTraceData(void* traceBuf, uint32_t traceBufSz, uint32_t numSamples, uint64_t ipBaseAddress, uint32_t& wordsPerSample)
  {
    ...
    if (numWords > chunkSizeWords) {
      for (; words < (numWords-chunkSizeWords); words += chunkSizeWords) {
        ...
        #endif
        unmgd_pread(0 /*flags*/, (void *)(hostbuf + words) /*buf*/, chunkSizeBytes /*count*/, ipBaseAddress + AXI_FIFO_RDFD_AXI_FULL /*offset : or AXI_FIFO_RDFD*/);
                      ...
                      uint32_t& traceSamples, uint32_t& traceBufSz)
{
  ...
  auto shim = get_shim_object(handle);
  return shim->xclGetTraceBufferInfo(nSamples, traceSamples, traceBufSz);
}
...
{  
  ...
  auto shim = get_shim_object(handle);
  return shim->xclReadTraceData(traceBuf, traceBufSz, numSamples, ipBaseAddress, wordsPerSample);
}
